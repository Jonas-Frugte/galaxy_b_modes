...
Output directory comes from config (MASS_MAP).
"""
...
for lc in LIGHTCONES:
    CAT_NAME = f"real_cat_{lc}"
    filepaths = FilePaths(CAT_NAME=CAT_NAME)

    root_dir = hdfstream.open("cosma", "/")
    lc_dir = root_dir[
        f"FLAMINGO/{sim_name}/{sim_name}/healpix_maps/nside_{nside}/lightcone{lc}_shells/"
    ]
