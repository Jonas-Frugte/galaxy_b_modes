...
# assumed survey n(z), Smail-type. only used for mock generation.
Z0, BETA = 0.64, 1.5
_nz_unnorm = lambda z: z**2 * np.exp(-(z / Z0)**BETA)
_NORM = scipy.integrate.quad(_nz_unnorm, 1e-8, 20)[0]

def gal_dens_z(z):
    return _nz_unnorm(z) / _NORM
...
def z_sample(ngals: int, z_delta: float = None, cosmology: CosmologySpec = CosmologySpec()) -> np.ndarray:
    if z_delta == None:
        z_grid = np.linspace(1e-6, 5.0, 200000)
        cdf = np.cumsum(gal_dens_z(z_grid))
        cdf /= cdf[-1]
        zs = np.interp(rng.random(ngals), cdf, z_grid)
    else:
        zs = np.full(ngals, z_delta)

    chis = cosmology.astropy_cosmo.comoving_distance(zs).value          # Mpc
    return zs, chis
...
def save_mock(zs: np.ndarray, chis: np.ndarray, nhats: np.ndarray, shapes: np.ndarray,
    ...
    print(f"wrote {ngals} galaxies to {filepaths.SHELLS_RESOLVED}, "
