...
if GEMINI_HOME.exists():
    ...
    PRODUCTS = REPO / "products_gemini" # end use products, like graphs or lists that can be directly inspected
elif MAC_HOME.exists():
    HOME = MAC_HOME
    CAT_NAME = "real_cat"
    DATA = Path("/Users/Frugt001/Desktop/flamingo_data") / CAT_NAME
    REPO     = HOME / "galaxy_b_modes"
    PRODUCTS = REPO / "products" / CAT_NAME # end use products, like graphs or lists that can be directly inspected
...
def CL_BIN_PAIR_NAME(i: int, j: int) -> str:
    return f"bin{min(i, j)}x{max(i, j)}.txt"

# STUFF FOR SNR AND CLS CALC
NLB = 32
F_SKY_EUCLID = 0.36
N_GAL_EUCLID_PER_ARCMIN2 = 30.0
SIGMA_E_EUCLID = 0.30